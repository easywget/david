...
tools=("git" "nmap" "hydra" "curl" "hping3" "ntpd"  "seclists")  #sudo apt install uuid-runtime dig, 
...
install_tools() {
    for tool in "${tools[@]}"; do
        # Handle 'ntp' separately to check for the service
        if [ "$tool" == "ntp" ]; then
            if systemctl is-active --quiet ntp; then
                echo "NTP service is active. Skipping installation."
                continue
            else
                echo "NTP is not installed. Installing..."
                apt-get install "$tool" -y > /dev/null
                systemctl start ntp
                systemctl enable ntp
                if systemctl is-active --quiet ntp; then
                    echo "NTP installed successfully and service started."
                else
                    echo "Failed to install NTP. Exiting."
                    exit 1
                fi
                continue
            fi
        ...
        # For other tools, use the command -v to check for existence and install if not present
