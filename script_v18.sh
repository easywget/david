...
install_tools() {
    for tool in "${tools[@]}"; do
        ...
        if [ "$tool" == "seclists" ]; then
            echo "seclists is not installed. Cloning from GitHub..."
        ...
        #if tools is not install, install
...
hydra_choose_wordlist() {
    ...
    case "$wordlistChoice" in
        1)
            ...
            if [[ ! -f "$user" ]] || [[ ! -f "$pass" ]]; then
                echo "Seclist wordlists not found. Please ensure Seclists is installed and paths are correct."
                return 0
            fi
            ;;
        2)
    ...
    #echo "Using username wordlist at $user"
    #echo "Using password wordlist at $pass"
}
